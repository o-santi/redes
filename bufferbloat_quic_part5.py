...
def start_webserver(net):
    ...
    proc = h1.popen(f"./bin/quiche-server --cert cert.crt --key cert.key --listen {h1.IP()}:4433 --root .", shell=True)
...
def fetch_html(net):
    ...
    time_init = time()
    proc = h2.popen(f"./bin/quiche-client https://{h1.IP()}:4433 --no-verify")
...
def fetch_files(net, n):
    ...
    deltas = []
    for i in range(n):
        time_init = start_times[i]
        proc = processos[i]
        (output, error) = proc.communicate() 
        string = str(error)
        chunks = string.split(' ') #formato é tipo "00.0user 00.0system 0:00elapsed etcetc"
        elapsed = chunks[2]
        tempo = elapsed[:len(elapsed)-7]  #a palavra 'elapsed' tem 7 letras
        min_sec = tempo.split(":")
        delta = (float(min_sec[0]) * 60) + (float(min_sec[1]))
        deltas.append(delta)
